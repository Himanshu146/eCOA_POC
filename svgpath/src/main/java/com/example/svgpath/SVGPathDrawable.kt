...
import com.richpath.listener.OnSVGPathUpdatedListener
import com.richpath.pathparser.PathParser
import com.richpath.util.PathUtils
import com.richpath.model.Vector
import kotlin.math.min

class SVGPathDrawable(private val vector: Vector?, private val scaleType: ImageView.ScaleType): Drawable() {
    ...
    private fun listenToPathsUpdates() {
        val vector = vector ?: return
        for (path in vector.paths) {
            path.onSVGPathUpdatedListener = object : OnSVGPathUpdatedListener {
    ...
    fun addPath(path: Path) {
        if (path is SVGPath) {
            ...
            addPath(SVGPath(path))
        }
    }

    private fun addPath(path: SVGPath) {
        val vector = vector ?: return
    ...
    fun getTouchedPath(event: MotionEvent?): SVGPath? {
        val vector = vector ?: return null
